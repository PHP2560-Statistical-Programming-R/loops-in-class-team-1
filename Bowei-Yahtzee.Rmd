...
bubble_sort = function(vec) {
    no_passes = 0  # set counter for the sequence in a specific row
    while(1) {
        no_swaps = 0 # set break if the all the vectors are already arranged.
        for (j in 1 : (length(vec) - 1 - no_passes)) { #we start from the all numbers and will have smaller range if the maximum is settled.
            if (vec[j] > vec[j + 1]) { #compare each two vectors in the same row
                s = vec[j] #store the first element in the memory
                vec[j] = vec[j+1] #swap if the previous element is larger than later element
                vec[j+1] = s #store the bigger element for the next comparison
                no_swaps = no_swaps + 1 #count how many time does it swap in total
            }
        }
        no_passes = no_passes + 1 #after the largest number is settled, reduce the range for comparison
        if(no_swaps == 0) break #stop if no more swap happens
    }
    vec
}
bubble_sort(x[1,])
