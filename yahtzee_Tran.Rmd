...
```
bubble_sort = function(vec) { ## Create the function bubble_sort with input vec which is a vector of each row of the matrix
    no_passes = 0 ## Set the counter no_passes to 0 
    while(1) { ## First condition needs to be true for the loop to start
        no_swaps = 0 ## Set the counter no_swaps to 0 for every time you swap an element
        for (j in 1 : (length(vec) - 1 - no_passes)) { ## a loop to go through every column of each row
            if (vec[j] > vec[j + 1]) { ## Compare the two elements of a row which are next to each other
                s = vec[j] ## Store the smaller number if it is smaller
                vec[j] = vec[j+1] ## Swap smaller element in front of the bigger element
                vec[j+1] = s 
                no_swaps = no_swaps + 1 ## Move to the next swaps
            }
        }
        no_passes = no_passes + 1 ## Count the number of passes
        if(no_swaps == 0) break ## Condition to stop the loops
    }
    vec ## return the final result
}
...
if (x[1]==x[5]){
  ...
  hand="4ofKind"
} else if (x[1]==1 & sum(x)==15 | x[1]==2 & sum(x)==17){
  hand="LargeStraight"
